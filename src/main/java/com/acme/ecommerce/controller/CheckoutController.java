...
public class CheckoutController {
  ...
  String postCouponCode(Model model,
                        @Valid @ModelAttribute(value = "couponCode") CouponCode couponCode,
                        BindingResult result, RedirectAttributes attributes) {
    if (!result.hasErrors()) {
      sCart.setCouponCode(couponCode);
      if (!couponCode.getCode().equals("")) {
        attributes.addFlashAttribute("flash", new FlashMessage("Coupon applied!", SUCCESS));
  ...
  String checkoutConfirmation(Model model) {
    ...
    if (purchase != null) {
      ...
      model.addAttribute("creditCard",
          purchase.getCreditCardNumber().replaceAll("\\w(?=\\w{4})", "*"));
    } else {
      logger.error("No purchases Found!");
      return ("redirect:/error");
    }
