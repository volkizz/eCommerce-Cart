...
public class CartController {
  ...
  public RedirectView addToCart(@ModelAttribute(value = "productId") long productId,
    ...
    productService.checkAvailability(addProduct, quantity);
    if (addProduct != null) {
  ...
  public RedirectView updateCart(@ModelAttribute(value = "productId") long productId,
    ...
    productService.checkAvailability(updateProduct, newQuantity);
          ...
          if (pp.getProduct() != null) {
            if (pp.getProduct().getId().equals(productId)) {
              if (newQuantity > 0) {
                ...
                logger.debug(updateProduct.getName() + "quantity set to " + newQuantity);
                attributes.addFlashAttribute("flash",
                    new FlashMessage(updateProduct.getName() + "quantity set to " + newQuantity,
                        SUCCESS));
              } else {
                purchase.getProductPurchases().remove(pp);
                logger.debug(updateProduct.getName() + " was removed because quantity was set to "
                    + newQuantity);
                attributes.addFlashAttribute("flash", new FlashMessage(
                   updateProduct.getName() + " was removed because quantity was set to "
                        + newQuantity, FAILURE));
              }
              break;
            }
        ...
        for (ProductPurchase pp : purchase.getProductPurchases()) {
          if (pp.getProduct() != null) {
            if (pp.getProduct().getId().equals(productId)) {
              ...
              attributes.addFlashAttribute("flash",
                  new FlashMessage(updateProduct.getName() + " removed.", SUCCESS));
              break;
            }
          }
        }
